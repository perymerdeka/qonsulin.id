...
export default function AdminApp({ initialSlug, initialAuthenticated }: { initialSlug: string[]; initialAuthenticated: boolean }) {
  ...
  async function loadAll() {
    setLoading(true);
    setMessage("");
    console.log("[CMS Admin] Memuat data langsung dari database Supabase...");
    try {
      if (typeof window !== "undefined") {
        localStorage.removeItem("qonsulin_local_cms_store");
      }

      const api = await fetch("/api/admin/cms", { cache: "no-store" }).catch((err) => {
        console.error("[CMS Admin Load Network Error]:", err);
        return null;
      });

      if (!api) {
        setMessage("Gagal terhubung ke API server CMS. Periksa koneksi jaringan.");
        return;
      }
      ...
      setMessage(`Gagal memuat dari database: ${body.message || "Pastikan sesi login administrator masih aktif."}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function save(sectionId: Exclude<Section, "dashboard" | "settings">, payload: Record<string, unknown>, id?: string) {
    ...
    try {
      ...
      if (!api) {
        throw new Error("Gagal terhubung ke API server CMS.");
      }

      const body = await api.json().catch(() => ({})) as CmsApiResponse & { message?: string };
      if (!api.ok || !body.row) {
        throw new Error(body.message || "Gagal menyimpan ke database Supabase.");
      }
      ...
      setMessage("Konten berhasil disimpan langsung ke database Supabase.");
      navigate(sectionId);
    } catch (error) {
      console.error("[CMS Admin Exception]:", error);
      setMessage(error instanceof Error ? error.message : "Gagal menyimpan data ke database.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function remove(sectionId: Exclude<Section, "dashboard" | "settings">, id: string) {
    if (!window.confirm("Hapus konten ini secara permanen dari database?")) return;
    setLoading(true);
    try {
      ...
      const res = api ? await api.json().catch(() => ({})) : null;
      if (!api?.ok || !res?.ok) {
        throw new Error(res?.message || "Gagal menghapus data dari database Supabase.");
      }
    ...
    <main className="admin admin-login">
      <div className="login-card">
        ...
        <div className="login-panel">
          ...
          <div className="live"><ShieldCheck size={16} />Terhubung Langsung ke Database Supabase</div>
        </div>
    ...
    <div className="settings-stack">
      ...
      <div className="admin-grid">
        <article className="admin-card">
          ...
          <p>Status Sambungan: <strong>Terhubung Langsung ke Supabase PostgreSQL (Fallback Dinonaktifkan)</strong></p>
